...
export async function GET(req: NextRequest) {
  try {
    ...
    const orders = await prisma.order.findMany({
      ...
      select: { paid_at: true, total_amount: true, payment_mode: true },
    });
    ...
    orders.forEach(order => {
      if (order.paid_at) {
        ...
        const idx = hour - 6;
        if (idx >= 0 && idx < 18) {
          hourlyData[idx].revenue += order.total_amount;
    ...
    orders.forEach(order => {
      if (order.paid_at) {
        ...
        dailyMap.set(dateKey, (dailyMap.get(dateKey) || 0) + order.total_amount);
      }
    });
    ...
    orders.forEach(order => {
      if (order.payment_mode) {
        paymentSplit[order.payment_mode as keyof typeof paymentSplit] += order.total_amount;
      }
    });

  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: 'var(--font-heading)', color: '#1A1A1A' }}>Reports</h1>
          <p className="text-sm mt-1" style={{ color: '#6B7280' }}>Revenue and sales analytics</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Revenue', value: `₹${(reportData?.total_revenue || 0).toLocaleString('en-IN')}`, icon: DollarSign, color: '#10B981' },
          ...
          <div key={card.label} className="rounded-xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        {/* Revenue Chart */}
        <div className="col-span-2 rounded-xl p-6" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB' }}>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={period === 'today' ? reportData?.hourly : reportData?.daily}>
              ...
              <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', fontSize: '12px' }} formatter={(v: any) => [`₹${v}`, 'Revenue']} />
